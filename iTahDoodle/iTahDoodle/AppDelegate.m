...
NSString *docPath()
{
    ...
    return [[pathList objectAtIndex:0] stringByAppendingPathComponent:@"date.dtd"];
}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    //  CGRect() creates from (x, y, width, height)
    ...
    //  Create and configure the next field where new task will be typed
...
{
    //  To improve the performance, we reconfigure cells in memory
    //  that have scrolled off the screen and hand them back
    //  with new contents instead of always creating new cells
    //  Firs we check to see if there's a cell available for reuse
...
- (void)addTask:(id)sender
{
    ...
    //  Add it to our working directory
    [tasks addObject:t];
